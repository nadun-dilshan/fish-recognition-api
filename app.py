...
# Load ontology for ENV_REQUIREMENTS (replaces JSON loading)
print("Loading ontology...")
onto = get_ontology("file://./owl/fishontology.owl").load()
with onto:
    sync_reasoner()  # Run the reasoner (even if no rules, for completeness)
...
def calculate_survival_probability(species, measurements):
    # Fetch species from ontology
    species_ind = onto.search_one(iri="*" + species)
    if species_ind is None:
        return {'probability': 0.0, 'issues': ['Unknown species requirements']}

    # Extract requirements using ontology properties
    req = {}
    req['pH'] = {
        'min': species_ind.phMin,
        'max': species_ind.phMax,
        'optimal': [species_ind.phOptimalMin, species_ind.phOptimalMax]
    }
    req['temperature_C'] = {
        'min': species_ind.tempMin,
        'max': species_ind.tempMax,
        'optimal': [species_ind.tempOptimalMin, species_ind.tempOptimalMax]
    }
    req['dissolved_oxygen_mgL'] = {
        'min': species_ind.doMin,
        'optimal': species_ind.doOptimal
    }
...
def health():
    return jsonify({
        ...
        'data_files': {
            ...
            'env_requirements': bool(onto),  # Updated to check ontology
